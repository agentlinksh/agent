...
1. **The live database** (local or cloud) — via `agentlink db apply` or `psql`, so changes take effect immediately
...
**Apply methods:**
- **Batch (recommended):** `agentlink db apply` — applies all schema files with correct ordering via `pgdelta`. DB URL auto-resolved from `.env.local`.
...
2. Applies via `agentlink db apply` — every file write must be followed by an apply
...
**4. Generate types:**
Types are auto-generated by `db apply`. To regenerate manually: `agentlink db types`.
