...
- `npx create-agentlink@latest db apply` — applies all schema files with `pgdelta declarative apply`, which resolves statement ordering automatically. **This is the only command the agent uses during development.**
- `npx create-agentlink@latest db migrate name` — generates migrations by comparing catalog snapshots (no shadow DB needed). **Deployment only — used when the user explicitly asks.**
