...
Interactive wizard that: checks prerequisites, starts Supabase, sets up the database (extensions, schemas, functions, queues, auth triggers), writes template files, generates migrations, configures Claude Code, installs MCP + plugin + companion skills, then launches Claude Code with the prompt.
...
| `--no-frontend` | Skip NextJS frontend scaffolding |
...
Post-setup migrations are written *after* `db diff` runs and marked as applied via `supabase migration repair --status applied --local`.

### Tier 2: Application migrations (generated by `db diff`)

```bash
supabase db diff --use-pg-delta -f descriptive_name
```

Captures everything in `public` and `api` schemas: tables, functions, indexes, policies, triggers. This is the standard migration workflow described in the `database` skill.
...
2. **Mark it as applied** — `supabase migration repair <version> --status applied --local`
