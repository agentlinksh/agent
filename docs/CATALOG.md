...
**Plugin:** `agentlink` — skills namespaced under `agentlink:`, agent available as `agentlink:builder`.
...
**Agent + composable skills** — The `builder` agent bundles all domain skills with prerequisites and architecture enforcement. Skills can also be used individually. A task like "add a new entity with RLS and an edge function" uses three skills at once, each contributing its domain.
...
├── agents/
│   └── builder.md         # Agent — prereqs, architecture, core rules
├── hooks/
